// Query batch 23 leads: type=studio, status=new OR status=active
...
async function queryStudioBatch23() {
  console.log('Querying studio leads with status=new OR status=active (offset 60, limit 10)...\n');
  ...
  // Get batch (offset 60, limit 10)
  const batch = leads.slice(60, 70);
  
  console.log(`Total studio leads (new/active): ${leads.length}`);
  console.log(`Batch 23 (60-69): ${batch.length} leads\n`);
  
  batch.forEach((lead, i) => {
    console.log(`${i + 60}. ${lead.name} (${lead.id})`);
...
queryStudioBatch23().then(() => process.exit(0)).catch(err => {
