// Query batch 11 leads (100-109) from Firestore
...
async function queryBatch11() {
  console.log('Querying investor leads for batch 11 (offset 100, limit 10)...\n');
  ...
  // Get batch 11 (offset 100, limit 10)
  const batch11 = leads.slice(100, 110);
  
  console.log(`Total leads needing research: ${leads.length}`);
  console.log(`Batch 11 (100-109): ${batch11.length} leads\n`);
  
  batch11.forEach((lead, i) => {
    console.log(`${100 + i}. ${lead.name} (${lead.id})`);
  ...
  console.log('\n=== JSON OUTPUT ===');
  console.log(JSON.stringify(batch11.map(l => ({
    id: l.id,
    name: l.name,
    website: l.website,
    country: l.country,
...
queryBatch11().then(() => process.exit(0)).catch(err => {
