// Query leads 20-29 for batch 3 research
...
async function getAllInvestorLeadsNeedingResearch() {
  ...
  // Get batch 3 (offset 20, limit 10)
  const batch3 = needsResearch.slice(20, 30);
  
  console.log(`\nBatch 3 (leads 20-29): ${batch3.length} leads\n`);
  
  batch3.forEach((lead, i) => {
    console.log(`${i+20}. ${lead.name || 'UNNAMED'} (${lead.id})`);
  ...
  console.log('\n=== JSON OUTPUT ===');
  console.log(JSON.stringify(batch3.map(l => ({
    id: l.id,
    name: l.name,
    website: l.website,
    country: l.country,
