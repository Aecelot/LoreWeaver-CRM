// Query batch 19 leads: type=studio, status=new OR status=active
...
async function queryStudioBatch19() {
  console.log('Querying studio leads with status=new OR status=active (offset 20, limit 10)...\n');
  ...
  // Get batch (offset 20, limit 10)
  const batch = leads.slice(20, 30);
  
  console.log(`Total studio leads (new/active): ${leads.length}`);
  console.log(`Batch 19 (20-29): ${batch.length} leads\n`);
  
  batch.forEach((lead, i) => {
    console.log(`${i + 20}. ${lead.name} (${lead.id})`);
...
queryStudioBatch19().then(() => process.exit(0)).catch(err => {
