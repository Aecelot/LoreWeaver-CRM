// Query batch 26 leads: type=studio, status=new OR status=active
...
async function queryStudioBatch26() {
  console.log('Querying studio leads with status=new OR status=active (offset 90, limit 10)...\n');
  ...
  // Get batch (offset 90, limit 10)
  const batch = leads.slice(90, 100);
  
  console.log(`Total studio leads (new/active): ${leads.length}`);
  console.log(`Batch 26 (90-99): ${batch.length} leads\n`);
  
  batch.forEach((lead, i) => {
    console.log(`${i + 90}. ${lead.name} (${lead.id})`);
...
queryStudioBatch26().then(() => process.exit(0)).catch(err => {
