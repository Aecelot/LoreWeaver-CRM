// Query batch 27 leads: type=studio, status=new OR status=active
...
async function queryStudioBatch27() {
  console.log('Querying studio leads with status=new OR status=active (offset 100, limit 10)...\n');
  ...
  // Get batch (offset 100, limit 10)
  const batch = leads.slice(100, 110);
  
  console.log(`Total studio leads (new/active): ${leads.length}`);
  console.log(`Batch 27 (100-109): ${batch.length} leads\n`);
  
  batch.forEach((lead, i) => {
    console.log(`${i + 100}. ${lead.name} (${lead.id})`);
...
queryStudioBatch27().then(() => process.exit(0)).catch(err => {
