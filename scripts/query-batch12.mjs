// Query batch 12 leads (110-119) from Firestore
...
async function queryBatch12() {
  console.log('Querying investor leads for batch 12 (offset 110, limit 10)...\n');
  ...
  // Get batch 12 (offset 110, limit 10)
  const batch12 = leads.slice(110, 120);
  
  console.log(`Total leads needing research: ${leads.length}`);
  console.log(`Batch 12 (110-119): ${batch12.length} leads\n`);
  
  batch12.forEach((lead, i) => {
    console.log(`${110 + i}. ${lead.name} (${lead.id})`);
  ...
  console.log('\n=== JSON OUTPUT ===');
  console.log(JSON.stringify(batch12.map(l => ({
    id: l.id,
    name: l.name,
    website: l.website,
    country: l.country,
...
queryBatch12().then(() => process.exit(0)).catch(err => {
