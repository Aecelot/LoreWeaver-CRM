// Query batch 14 leads (130-139) from Firestore
...
async function queryBatch14() {
  console.log('Querying investor leads for batch 14 (offset 130, limit 10)...\n');
  ...
  // Get batch 14 (offset 130, limit 10)
  const batch14 = leads.slice(130, 140);
  
  console.log(`Total leads needing research: ${leads.length}`);
  console.log(`Batch 14 (130-139): ${batch14.length} leads\n`);
  
  batch14.forEach((lead, i) => {
    console.log(`${130 + i}. ${lead.name} (${lead.id})`);
  ...
  console.log('\n=== JSON OUTPUT ===');
  console.log(JSON.stringify(batch14.map(l => ({
    id: l.id,
    name: l.name,
    website: l.website,
    country: l.country,
...
queryBatch14().then(() => process.exit(0)).catch(err => {
