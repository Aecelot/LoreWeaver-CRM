// Query batch 10 leads (90-99) from Firestore
...
async function queryBatch10() {
  console.log('Querying investor leads for batch 10 (offset 90, limit 10)...\n');
  ...
  // Get batch 10 (offset 90, limit 10)
  const batch10 = leads.slice(90, 100);
  
  console.log(`Total leads needing research: ${leads.length}`);
  console.log(`Batch 10 (90-99): ${batch10.length} leads\n`);
  
  batch10.forEach((lead, i) => {
    console.log(`${90 + i}. ${lead.name} (${lead.id})`);
  ...
  console.log('\n=== JSON OUTPUT ===');
  console.log(JSON.stringify(batch10.map(l => ({
    id: l.id,
    name: l.name,
    website: l.website,
    country: l.country,
...
queryBatch10().then(() => process.exit(0)).catch(err => {
