// Query batch 25 leads: type=studio, status=new OR status=active
...
async function queryStudioBatch25() {
  console.log('Querying studio leads with status=new OR status=active (offset 80, limit 10)...\n');
  ...
  // Get batch (offset 80, limit 10)
  const batch = leads.slice(80, 90);
  
  console.log(`Total studio leads (new/active): ${leads.length}`);
  console.log(`Batch 25 (80-89): ${batch.length} leads\n`);
  
  batch.forEach((lead, i) => {
    console.log(`${i + 80}. ${lead.name} (${lead.id})`);
...
queryStudioBatch25().then(() => process.exit(0)).catch(err => {
