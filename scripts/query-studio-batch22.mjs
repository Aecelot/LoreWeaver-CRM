// Query batch 22 leads: type=studio, status=new OR status=active
...
async function queryStudioBatch22() {
  console.log('Querying studio leads with status=new OR status=active (offset 50, limit 10)...\n');
  ...
  // Get batch (offset 50, limit 10)
  const batch = leads.slice(50, 60);
  
  console.log(`Total studio leads (new/active): ${leads.length}`);
  console.log(`Batch 22 (50-59): ${batch.length} leads\n`);
  
  batch.forEach((lead, i) => {
    console.log(`${i + 50}. ${lead.name} (${lead.id})`);
...
queryStudioBatch22().then(() => process.exit(0)).catch(err => {
