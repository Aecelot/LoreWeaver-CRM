// Query batch 28 leads: type=studio, status=new OR status=active
...
async function queryStudioBatch28() {
  console.log('Querying studio leads with status=new OR status=active (offset 110, limit 10)...\n');
  ...
  // Get batch (offset 110, limit 10)
  const batch = leads.slice(110, 120);
  
  console.log(`Total studio leads (new/active): ${leads.length}`);
  console.log(`Batch 28 (110-119): ${batch.length} leads\n`);
  
  batch.forEach((lead, i) => {
    console.log(`${i + 110}. ${lead.name} (${lead.id})`);
...
queryStudioBatch28().then(() => process.exit(0)).catch(err => {
