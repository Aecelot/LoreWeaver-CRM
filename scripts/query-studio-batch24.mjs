// Query batch 24 leads: type=studio, status=new OR status=active
...
async function queryStudioBatch24() {
  console.log('Querying studio leads with status=new OR status=active (offset 70, limit 10)...\n');
  ...
  // Get batch (offset 70, limit 10)
  const batch = leads.slice(70, 80);
  
  console.log(`Total studio leads (new/active): ${leads.length}`);
  console.log(`Batch 24 (70-79): ${batch.length} leads\n`);
  
  batch.forEach((lead, i) => {
    console.log(`${i + 70}. ${lead.name} (${lead.id})`);
...
queryStudioBatch24().then(() => process.exit(0)).catch(err => {
