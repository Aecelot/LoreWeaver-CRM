// Query batch 18 leads: type=studio, status=new OR status=active
...
async function queryStudioBatch18() {
  console.log('Querying studio leads with status=new OR status=active (offset 10, limit 10)...\n');
  ...
  // Get batch (offset 10, limit 10)
  const batch = leads.slice(10, 20);
  
  console.log(`Total studio leads (new/active): ${leads.length}`);
  console.log(`Batch 18 (10-19): ${batch.length} leads\n`);
  
  batch.forEach((lead, i) => {
    console.log(`${i + 10}. ${lead.name} (${lead.id})`);
...
queryStudioBatch18().then(() => process.exit(0)).catch(err => {
