// Query batch 20 leads: type=studio, status=new OR status=active
...
async function queryStudioBatch20() {
  console.log('Querying studio leads with status=new OR status=active (offset 30, limit 10)...\n');
  ...
  // Get batch (offset 30, limit 10)
  const batch = leads.slice(30, 40);
  
  console.log(`Total studio leads (new/active): ${leads.length}`);
  console.log(`Batch 20 (30-39): ${batch.length} leads\n`);
  
  batch.forEach((lead, i) => {
    console.log(`${i + 30}. ${lead.name} (${lead.id})`);
...
queryStudioBatch20().then(() => process.exit(0)).catch(err => {
